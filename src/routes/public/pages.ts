...
function mapPage(row:PageRow,useDraft=false){
 const content=pageModel(useDraft?row.content_json:row.published_content_json,row.id);
...
export async function publicPagesRoute(request:Request,env:Env):Promise<Response>{
 const url=new URL(request.url),slug=url.searchParams.get('slug')?.trim().toLowerCase()||'',explicitPreview=url.searchParams.get('preview')==='1';
 let useDraft=explicitPreview;
 ...
 if(!slug){const result=await env.DB.prepare(`SELECT id,slug,title,description,published_content_json,content_json,updated_at FROM pages ${useDraft?'':'WHERE is_published=1'} ORDER BY updated_at DESC,title ASC LIMIT 100`).all<PageRow>();const pages=result.results.map(r=>mapPage(r,useDraft)).filter(Boolean);return ok(pages)}
 const row=await env.DB.prepare(`SELECT id,slug,title,description,published_content_json,content_json,updated_at FROM pages WHERE slug=?1 ${useDraft?'':'AND is_published=1'} LIMIT 1`).bind(slug).first<PageRow>();
 if(!row)return error('PAGE_NOT_FOUND',404);
 const page=mapPage(row,useDraft);if(!page)return error('PAGE_NOT_CANONICAL',409,'Az oldal még nincs canonical Editor v2 Page Modelre migrálva.');return ok(page);
}
