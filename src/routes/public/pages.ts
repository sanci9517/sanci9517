...
function mapPage(row:PageRow,useDraft=false){const raw=parse(useDraft?row.content_json:row.published_content_json);const document=normalizeEditorDocument(raw,row.id,row.title,row.slug,row.description);if(!document)return null;const content=document.pages[row.id];return{id:row.id,slug:row.slug,title:row.title,description:row.description,content:{...content.pages[row.id],type:'sanci-document'},updatedAt:row.updated_at}}
export async function publicPagesRoute(request:Request,env:Env):Promise<Response>{
 ...
 if(!slug){const result=await env.DB.prepare(`SELECT id,slug,title,description,published_content_json,content_json,updated_at FROM pages ${useDraft?'':'WHERE is_published=1'} ORDER BY updated_at DESC,title ASC LIMIT 100`).all<PageRow>();return ok(result.results.map(r=>mapPage(r,useDraft)).filter(Boolean))}
 const row=await env.DB.prepare(`SELECT id,slug,title,description,published_content_json,content_json,updated_at FROM pages WHERE slug=?1 ${useDraft?'':'AND is_published=1'} LIMIT 1`).bind(slug).first<PageRow>();if(!row)return error('PAGE_NOT_FOUND',404);const page=mapPage(row,useDraft);if(!page)return error('PAGE_NOT_CANONICAL',409,'Az oldal dokumentuma nem alakítható canonical Editor v2 Page Modelre.');return ok(page);
}
