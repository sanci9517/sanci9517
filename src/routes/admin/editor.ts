...
function isCanonicalDocument(document: any, pageId: string): boolean {
  return Boolean(document && document.type === "sanci-document" && Array.isArray(document.pages) && document.pages.length === 1 && document.pages[0]?.id === pageId && document.activePageId === pageId);
}
...
async function ensurePage(env: Env, id: string, document?: any) {
  ...
  await env.DB.prepare(`INSERT INTO pages (id,slug,title,description,content_json,published_content_json,is_published,updated_at) VALUES (?,?,?,?,?,?,0,CURRENT_TIMESTAMP)`)
...
export async function adminEditorRoute(request: Request, env: Env): Promise<Response> {
  ...
  if (request.method === "GET") {
    const page = await env.DB.prepare(`SELECT id,slug,title,description,content_json,is_published,updated_at FROM pages WHERE id=? LIMIT 1`)
      .bind(pageId).first<PageRow>();
    if (!page) return error("PAGE_NOT_FOUND", 404);

    const document = parse(page.content_json);
    if (!isCanonicalDocument(document, page.id)) {
      return error("PAGE_NOT_MIGRATED", 409, "Az oldal még nincs canonical Page Modelre migrálva.");
    }

    const revisions = await env.DB.prepare(`SELECT id,version,note,created_by,created_at,document_json FROM editor_revisions WHERE page_id=? ORDER BY version DESC LIMIT 30`)
      .bind(pageId).all();

    return ok({
      page: { id: page.id, slug: page.slug, title: page.title, description: page.description, isPublished: Boolean(page.is_published), updatedAt: page.updated_at, document },
      revisions: revisions.results.map((r: any) => ({ id: r.id, version: r.version, note: r.note, created_by: r.created_by, created_at: r.created_at }))
    });
  }
  ...
  if (request.method === "POST") {
    ...
    const statements = [
      ...
      env.DB.prepare(`INSERT INTO editor_revisions (id,page_id,version,document_json,created_by,note) VALUES (?,?,?,?,?,?)`).bind(revId, id, version, json, user.id, note)
    ];
  ...
  if (request.method === "PUT") {
    ...
    await env.DB.batch([
      ...
      env.DB.prepare(`INSERT INTO editor_revisions (id,page_id,version,document_json,created_by,note) VALUES (?,?,?,?,?,?)`).bind(crypto.randomUUID(), id, next, rev.document_json, user.id, `Rollback from v${version}`)
    ]);
