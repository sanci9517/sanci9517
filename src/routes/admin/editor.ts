...
function isCanonicalDocument(document:any,pageId:string){return Boolean(document&&document.type==='sanci-document'&&Array.isArray(document.pages)&&document.pages.length===1&&document.pages[0]?.id===pageId&&document.activePageId===pageId)}
export async function adminEditorRoute(request:Request,env:Env):Promise<Response>{
 ...
 if(request.method==='GET'){
  ...
  const document=parse(page.content_json);if(!isCanonicalDocument(document,page.id))return error('PAGE_NOT_MIGRATED',409,'Az oldal még nincs canonical Page Modelre migrálva.');
  ...
  return ok({page:{id:page.id,slug:page.slug,title:page.title,description:page.description,isPublished:Boolean(page.is_published),updatedAt:page.updated_at,document},revisions:revisions.results.map((r:any)=>({id:r.id,version:r.version,note:r.note,created_by:r.created_by,created_at:r.created_at}))});
 }
 if(request.method==='POST'){
  let body:any;try{body=await request.json()}catch{return error('INVALID_JSON',400)}
  const id=typeof body.pageId==='string'?body.pageId:pageId,document=body.document;if(!id||!document||typeof document!=='object')return error('INVALID_EDITOR_DOCUMENT',400);if(!isCanonicalDocument(document,id))return error('INVALID_EDITOR_DOCUMENT',400,'A mentett dokumentumnak pontosan egy oldalt kell tartalmaznia.');
