...
export async function adminEditorRoute(request:Request,env:Env):Promise<Response>{
 ...
 if(request.method==="POST"){
  ...
  const statements=[env.DB.prepare("INSERT INTO editor_revisions (id,page_id,version,document_json,created_by,note) VALUES (?,?,?,?,?,?)").bind(revId,id,version,json,user.id,typeof body.note==="string"?body.note.slice(0,200):publish?"Publikálás":"Editor mentés")];
  if(publish)statements.push(env.DB.prepare("UPDATE pages SET content_json=?,is_published=1,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(json,id));
 ...
 if(request.method==="PUT"){
  let body:any;try{body=await request.json()}catch{return error("INVALID_JSON",400)}const id=typeof body.pageId==="string"?body.pageId:pageId,version=Number(body.version);if(!id||!Number.isInteger(version)||version<1)return error("INVALID_REVISION",400);const rev=await env.DB.prepare("SELECT document_json FROM editor_revisions WHERE page_id=? AND version=? LIMIT 1").bind(id,version).first<{document_json:string}>();if(!rev)return error("REVISION_NOT_FOUND",404);const latest=await env.DB.prepare("SELECT COALESCE(MAX(version),0) AS version FROM editor_revisions WHERE page_id=?").bind(id).first<{version:number}>();const next=Number(latest?.version||0)+1,json=rev.document_json;await env.DB.batch([env.DB.prepare("UPDATE pages SET content_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(json,id),env.DB.prepare("INSERT INTO editor_revisions (id,page_id,version,document_json,created_by,note) VALUES (?,?,?,?,?,?)").bind(crypto.randomUUID(),id,next,json,user.id,`Rollback from v${version}`)]);return ok({pageId:id,version:next,restoredFrom:version});
 }
