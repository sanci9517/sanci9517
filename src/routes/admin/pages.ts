...
export async function adminPagesRoute(request:Request,env:Env):Promise<Response>{
 ...
 await env.DB.prepare(`INSERT INTO editor_revisions (id,page_id,version,document_json,created_by,note) VALUES (?,?,?,?,?,?)`).bind(crypto.randomUUID(),id,1,contentJson,user.id,'Oldal létrehozva az Editor v2-ben').run();await audit(env,user.id,'page.create',id,{slug,title});return ok({id,slug,title,description,content:document,isPublished,version:1});
}
