...
type PageBody = { id?:unknown; slug?:unknown; title?:unknown; description?:unknown; document?:unknown; content?:unknown; isPublished?:unknown };
function parse(value:string):unknown{try{return JSON.parse(value)}catch{return {}}}
function canonical(value:unknown,id:string):boolean{return Boolean(value&&typeof value==='object'&&(value as any).type==='sanci-document'&&Array.isArray((value as any).pages)&&(value as any).pages.length===1&&(value as any).pages[0]?.id===id&&(value as any).activePageId===id)}
...
export async function adminPagesRoute(request:Request,env:Env):Promise<Response>{
 ...
 if(!canonical(document,id))return error("INVALID_EDITOR_DOCUMENT",400,"Az új oldalnak canonical Visual Editor dokumentumot kell tartalmaznia.");
 ...
 await env.DB.prepare(`INSERT INTO editor_revisions (id,page_id,version,document_json,created_by,note) VALUES (?,?,?,?,?,?)`).bind(crypto.randomUUID(),id,1,contentJson,user.id,'Oldal létrehozva a Visual Editorban').run();
