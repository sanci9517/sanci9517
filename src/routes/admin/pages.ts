...
function serialize(row: PageRow, includeContent = true) {
  return {
    id: row.id,
    slug: row.slug,
    title: row.title,
    description: row.description,
    ...(includeContent ? { content: parse(row.content_json) } : {}),
...
export async function adminPagesRoute(request: Request, env: Env): Promise<Response> {
  ...
  if (request.method === "GET") {
    ...
    return ok(rows.results.map(row => serialize(row, !metaOnly)));
  }

  if (request.method === "DELETE") {
    let body: PageBody;
    try { body = await request.json() as PageBody; } catch { return error("INVALID_JSON", 400); }
  ...
  if (request.method === "PATCH") {
    ...
    return ok(serialize({ ...existing, title }, true));
  }
  ...
  try {
    ...
    if (contentJson.length > 180000) return error("PAGE_TOO_LARGE", 400);
  } catch {
  ...
  if (request.method === "POST") {
    ...
    try {
      await env.DB.prepare(
        `INSERT INTO pages
         (id,slug,title,description,content_json,published_content_json,is_published,updated_at)
         VALUES (?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`
      ).bind(id, slug, title, description, contentJson, isPublished ? contentJson : null, isPublished ? 1 : 0).run();
    } catch (e) {
  ...
  if (request.method === "PUT") {
    ...
    try {
      await env.DB.prepare(
        `UPDATE pages SET slug=?,title=?,description=?,content_json=?,published_content_json=?,is_published=?,updated_at=CURRENT_TIMESTAMP
         WHERE id=?`
      ).bind(slug, title, description, contentJson, isPublished ? contentJson : null, isPublished ? 1 : 0, id).run();
    } catch (e) {
      if (String(e).toLowerCase().includes("unique")) return error("SLUG_EXISTS", 409, "Slug already exists");
      throw e;
    }
