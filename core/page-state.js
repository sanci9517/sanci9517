...
export function savePages(nextPages) { if (!Array.isArray(nextPages)) return false; return storage.set(PAGES_KEY, nextPages.map(normalizePage)); }
...
export function syncPagesFromNavigation(items) {
  ...
  for (const item of items) {
    ...
    const path = normalizePath(item.path); if (!path || path === '/') continue;
    const existing = byPath.get(path);
    next.push(existing ? normalizePage(existing) : normalizePage({ id: createPageId(path), path, title: String(item.label || 'Új oldal').trim() || 'Új oldal', status: 'active', menu: true, content: '', blocks: [] }));
  }
