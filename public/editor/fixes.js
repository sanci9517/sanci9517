(()=>{
  ...
  const normalize=v=>String(v??'').toLocaleLowerCase('hu-HU').normalize('NFD').replace(/[\u0300-\u036f]/g,'').trim();
  let searchRun=0;
  const applySearch=(input,token)=>{
    if(token!==searchRun)return;
    const q=normalize(input?.value||'');
    const buttons=[...document.querySelectorAll('.element[data-add]')];
    let count=0;
    buttons.forEach(btn=>{
      const type=normalize(btn.dataset.add);
      const label=normalize(btn.querySelector('span')?.textContent);
      const title=normalize(btn.title);
      const info=normalize(btn.querySelector('.element-info')?.getAttribute('aria-label'));
      const match=!q||`${type} ${label} ${title} ${info}`.includes(q);
      btn.hidden=!match;
      if(match)count++;
    });
  ...
  const bindElementSearch=()=>{
    ...
    const run=()=>{
      const token=++searchRun;
      if(typeof window.populate==='function')window.populate('');
      requestAnimationFrame(()=>requestAnimationFrame(()=>applySearch(input,token)));
    };
