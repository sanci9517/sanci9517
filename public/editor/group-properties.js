(() => {
  ...
  const presets={
    position:[['absolute','Abszolút'],['relative','Relatív'],['static','Normál']],
    display:[['block','Blokk'],['flex','Flex'],['grid','Rács'],['inline-block','Inline blokk']],
    border:[['none','Nincs'],['1px solid rgba(255,255,255,.12)','Finom'],['1px solid rgba(255,255,255,.22)','Normál'],['2px solid rgba(255,255,255,.35)','Erős'],['1px solid #ffffff','Világos'],['1px solid #111827','Sötét']],
    radius:[['0','Nincs'],['6','Enyhe'],['10','Normál'],['16','Kerek'],['24','Erősen kerek']],
    opacity:[['1','100%'],['0.9','90%'],['0.8','80%'],['0.6','60%'],['0.5','50%']]
  };
  function render(){
    const box=document.getElementById('inspector');if(!box)return;const ns=nodes(),is=grouped(ns);
    const sig=is?ns.map(n=>n.id).sort().join('|'):'none';
    if(!is){lastSignature='none';return}
    if(sig===lastSignature&&box.querySelector('.group-properties'))return;
    lastSignature=sig;
    const l=p=>common(ns,p);
    box.innerHTML=`<div class="group-properties"><div class="group-title">Csoport tulajdonságai</div><div class="group-subtitle">Ugyanaz a logika, mint az egy elem tulajdonságainál. A módosítás minden csoporttagra érvényes.</div>
  ...
  function apply(path,value){
    const a=api(),ns=nodes();if(!a||!grouped(ns)||!a.commit||!a.render||!a.setPath)return;a.commit();
    if(path==='layout.x'||path==='layout.y'){const axis=path==='layout.x'?'x':'y',current=Math.min(...ns.map(n=>Number(n.layout?.[axis])||0)),delta=Number(value)-current;ns.forEach(n=>{n.layout=n.layout||{};n.layout[axis]=(Number(n.layout[axis])||0)+delta})}
    else if(path==='layout.width'||path==='layout.height'){const axis=path==='layout.width'?'width':'height',pos=axis==='width'?'x':'y',min=Math.min(...ns.map(n=>Number(n.layout?.[pos])||0)),max=Math.max(...ns.map(n=>(Number(n.layout?.[pos])||0)+(Number(n.layout?.[axis])||0))),old=Math.max(1,max-min),target=Math.max(20,Number(value)||20),scale=target/old;ns.forEach(n=>{n.layout=n.layout||{};const p=Number(n.layout[pos])||0,s=Number(n.layout[axis])||0;n.layout[pos]=min+(p-min)*scale;n.layout[axis]=Math.max(20,s*scale)})}
  ...
  const observer=new MutationObserver(()=>requestAnimationFrame(render));observer.observe(document.getElementById('canvas')||document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
