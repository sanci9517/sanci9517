(() => {
  const info = {
    ...
    section:['Szakasz','Az oldal egy nagyobb, önálló tartalmi része.'], container:['Konténer','Tartalom szélességének és elhelyezésének rendezésére.'], row:['Sor','Ele­mek vízszintes sorba rendezésére.'], columns:['Oszlopok','Több tartalmi oszlop létrehozására.'], grid:['Rács','Tartalom szabályos rácsos elrendezésére.'], stack:['Egymásra rendezés','Ele­mek függőleges egymás alá rendezésére.'], flex:['Rugalmas elrendezés','Flex alapú, rugalmas elemrendezéshez.'], group:['Csoport','Több elem együttes kezelésére.'],
  ...
  const wait = () => {
    document.querySelectorAll('.element[data-add]').forEach(btn => {
      ...
      if (small) small.textContent = key === 'popup' ? 'felugró funkció' : 'elem';
  ...
  style.textContent = `.element{position:relative}.element-info{position:absolute;right:7px;top:50%;transform:translateY(-50%);width:20px;height:20px;border:1px solid currentColor;border-radius:50%;background:transparent;color:inherit;opacity:.7;font-weight:700;cursor:pointer}.element-info:hover{opacity:1}.element-popover{position:absolute;z-index:9999;left:calc(100% + 8px);top:0;width:250px;padding:12px;border:1px solid rgba(255,255,255,.14);border-radius:10px;background:#151922;box-shadow:0 12px 30px rgba(0,0,0,.35);color:#fff;text-align:left}.element-popover strong{display:block;margin-bottom:5px}.element-popover p{margin:0 0 10px;font-size:12px;line-height:1.45;color:#b8c0cc}.element-popover button{border:0;border-radius:6px;padding:5px 8px;background:#2a3140;color:#fff;cursor:pointer}`;
