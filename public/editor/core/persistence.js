...
function canonicalFromContent(item){const c=item?.content;if(!c||typeof c!=='object'||Array.isArray(c))return null;if(c.type==='sanci-document'&&Array.isArray(c.pages))return c;return null}
function pageFromServer(item){
 const revision=item?.editorDocument;
 const canonical=canonicalFromContent(item);
 let page=null;
 if(revision&&Array.isArray(revision.pages))page=revision.pages.find(p=>p?.id===item.id)||null;
 if(!page&&canonical&&Array.isArray(canonical.pages))page=canonical.pages.find(p=>p?.id===item.id)||null;
 if(!page&&revision&&Array.isArray(revision.pages)&&revision.pages.length===1){page=Schema.clone(revision.pages[0]);page.id=item.id}
 if(!page&&canonical&&Array.isArray(canonical.pages)&&canonical.pages.length===1){page=Schema.clone(canonical.pages[0]);page.id=item.id}
...
async function load(){
 try{
  setState('● Oldalak és tartalom betöltése…');const previous=activePage(),previousId=previous?.id||null;
  ...
  const loaded=pages.map(pageFromServer),preferred=loaded.find(p=>p.id===previousId),withContent=loaded.find(p=>(p.root?.children||[]).length>0),active=preferred||withContent||loaded[0];
  const document={schemaVersion:Schema.VERSION,type:'sanci-document',pages:loaded,activePageId:active.id};const validation=Schema.validate(document);if(!validation.valid)throw new Error(validation.errors.join(' '));
  E.load(document);window.SanciEditorUI?.render();setState(`✓ ${loaded.length} oldal betöltve D1-ből`);
...
async function save(publish){
 ...
 try{const document=E.getState().document,page=activePage();if(!page)throw new Error('Nincs aktív oldal.');const validation=Schema.validate(document);if(!validation.valid)throw new Error(validation.errors.join(' '));const pageDocument={schemaVersion:Schema.VERSION,type:'sanci-document',pages:[Schema.clone(page)],activePageId:page.id};const result=await request('POST',page.id,{pageId:page.id,document:pageDocument,note:publish?'Publikálás':'Editor mentés',publish});await pagesRequest('PATCH',{id:page.id,title:page.name||'Új oldal'}).catch(()=>null);E.getState().dirty=false;setState(`✓ Szerverre mentve · v${result.version}`);toast(publish?'Publikálva és D1-be mentve':'D1-be mentve');return result}catch(err){setState('⚠ Mentés sikertelen');toast(err.message||'Szerveroldali mentés sikertelen');throw err}finally{busy=false}}
