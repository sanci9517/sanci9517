...
(function(){
  function renderNode(node,selected,depth=0){
    ...
    if(children.length){
      ...
      children.forEach(child=>wrap.appendChild(renderNode(child,selected,depth+1)));
  ...
  function render(state,root){
    ...
    (state.document?.page?.root?.children||[]).forEach(n=>root.appendChild(renderNode(n,selected,0)));
  }
