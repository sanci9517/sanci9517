...
(function(){
  ...
  const labels={section:'Szakasz',container:'Konténer',text:'Szöveg',heading:'Címsor',paragraph:'Bekezdés',button:'Gomb',image:'Kép',card:'Kártya',group:'Csoport',custom:'Importált tartalom'};
  ...
  function renderNode(node,selected){
    ...
    if(node.type==='custom'&&node.content?.html){const frame=document.createElement('iframe');frame.className='legacy-content-frame';frame.setAttribute('sandbox','allow-same-origin');frame.setAttribute('title',node.name||'Importált oldal tartalma');frame.srcdoc='<!doctype html><html lang="hu"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'+node.content.html+'</head><body></body></html>';frame.style.width='100%';frame.style.height='100%';frame.style.border='0';frame.style.display='block';frame.style.pointerEvents='none';el.appendChild(frame);return el}
