(()=>{
  ...
  document.addEventListener('pointerdown',e=>{
    ...
    if(!node){
      if(!e.ctrlKey&&!e.metaKey&&!e.shiftKey)clear();
      return;
    }
  ...
  document.addEventListener('click',e=>{
    const node=nodeFromEvent(e);
    if(node){
      if(e.ctrlKey||e.metaKey||e.shiftKey){e.preventDefault();e.stopImmediatePropagation();return;}
      selected.clear();selected.add(node.dataset.id);paint();return;
    }
    if(!e.ctrlKey&&!e.metaKey&&!e.shiftKey)clear();
  },true);
