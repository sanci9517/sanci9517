...
-- One-time conversion of legacy page rows into the canonical Visual Editor Page Model.
...
WHERE id LIKE 'legacy-%'
  AND json_extract(content_json, '$.type') = 'sanci-document';

INSERT OR IGNORE INTO editor_revisions (id,page_id,version,document_json,created_by,note)
SELECT 'migration-20260916-' || slug || '-v1', id, 1, content_json, NULL,
