...
export function createRouteDispatcher({ json, corsHeaders, isTwitchConfigured, twitch }) {
  return async function dispatch(request, env) {
    ...
    if (url.pathname === '/admin/audit' && request.method === 'GET') return handleAdminAudit(request, env, json);
    ...
    if (request.method === 'GET' && url.pathname === '/health/storage') {
      try { const storage = await getStorageHealth(env); return json({ ok: storage.d1 && storage.kv, storage, checkedAt: new Date().toISOString() }, 200, {}, request, env); }
      catch (error) { console.error('[Sanci9517] Storage health error:', error); return json({ ok: false, storage: { d1: false, kv: false }, error: 'Storage health check failed.' }, 503, {}, request, env); }
    }
